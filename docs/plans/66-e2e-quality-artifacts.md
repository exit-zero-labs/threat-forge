...
| 2026-07-25 (implementation self-review) | Tightened runtime bounds, portability, and board truth | Self-review corrected the rolling console buffer so it evicts old entries before admitting a complete newest message and proves size/redaction behavior in committed tests. Accessibility evidence now bounds target/help/URL fields and strips malformed-URL query data. Manifest paths accept contained Windows paths, resolve symlinks before read/stat, expose external/missing-file state, sanitize context read errors, validate positive integer viewports, reject duplicate contexts, and keep `dirty: null` when git state is unknowable. The CLI report-read guard no longer swallows transform defects. Canonical axe lock entries were generated by npm from independently corroborated public package metadata after direct registry transport failed; the generic lock checker passes. #220 was restored from false `Ready` to Backlog because its HITL decision and M-size plan remain unresolved. |
| 2026-07-25 (verification clarification) | Distinguished unexpected failures from expected policy proofs | A real unexpected-failure probe confirmed screenshot, trace, video, context, console, accessibility, and DOM evidence. Playwright intentionally omitted trace/video for `test.fail()` policy proofs whose outer status is expected; those attempts still receive #66's context/console/accessibility/DOM evidence and do not trigger #183's upload gate. The runbook now states that distinction rather than promising built-in trace/video for expected failures. |
| 2026-07-25 (preflight corrections) | Closed classification, status, viewport, and fail-open proof gaps | Preflight made visual-diff classification extension-aware (`-expected.png`/`-actual.png`/`-diff.png`), expanded diagnostic capture to actual/expected status mismatches so an unexpected pass of `test.fail()` keeps evidence, strengthened 900×700 checks to require full viewport intersection, and replaced raw report-path/parser errors with fixed portable reasons. Manifest input now rejects valid JSON without a `suites` array, artifact-context requires schemaVersion 1, and symlink escapes are distinguished from missing files and omitted as external paths. |
| 2026-07-25 (preflight convergence) | Corrected cross-host paths and lazy-fixture claims | Segment-aware containment now accepts legitimate `..artifacts` names while rejecting only actual parent traversal; Windows/UNC report labels use cross-platform basename handling and cannot leak host paths. Documentation now scopes context/console/accessibility guarantees to page-backed attempts and records project-default fallback for browser-free support tests. Manifest tests pin the exact 12 interaction prefixes instead of iterating the implementation's own list, and accessibility audit prose no longer claims a passing scan proves every configured exception was observed. |
