...
| 2026-07-24 (implementation self-review) | Closed the real connection blocker and strengthened discriminating evidence | A real `connectElements` workflow proved the plan's strict-mode target assumption false: overlapping source/target handles made target hit-testing impossible. The defect was split as #213 and landed independently in `b68f24f` before #65 resumed; no DOM dispatch or store shortcut was accepted. Self-review also fixed four success-shaped gaps: all seed functions now anchor page-side timestamps and return only after their advertised state settles; large render time is measured from file handoff rather than implied by a locator timeout; the realistic fixture asserts boundaries and identified/mitigated threats as well as nodes/flows; and the write-failure test counts exactly one warning. One real workflow exercises every shared interaction helper, and expected-failure cases prove all four failure channels (error, warning, page error, failed request). The dedicated spec therefore contains 14 tests rather than the original 10; no existing assertion was removed. A scoped TypeScript check also corrected the Playwright setting from a silently ignored top-level `use.reducedMotion` property to the typed `use.contextOptions.reducedMotion` boundary; permanent E2E lint/type enforcement is tracked separately in #215. |
| 2026-07-24 (independent preflight) | Made fixture and exception proofs channel- and state-complete | Preflight found that a message-only exception could suppress identical text on `console.error`, `pageerror`, or `requestfailed`. All earlier `allowedConsolePatterns` references are superseded by `allowedBrowserEvents: [{ channel, pattern }]`; a same-message/different-channel expected-failure test proves the warning exception cannot cross channels. Realistic and large seeds now await their full defining topology before returning, and the 5-second large bound is sampled only after all 150 nodes and 100 edges render. The localStorage guard now claims only the opaque wrapper subset it consumes, the empty fixture proves a mounted model canvas rather than the pre-model state, and stale-manifest seeding returns a storage readback proving the two-entry pre-reload state before reconciliation leaves one real tab. The dedicated spec now has 15 tests. Because the branch adds a `.thf` fixture, `.github/instructions/file-formats.instructions.md` overrides the initial specialist applicability assessment: the threat-model-expert lane is required before handoff even though no schema or STRIDE behavior changes. |
