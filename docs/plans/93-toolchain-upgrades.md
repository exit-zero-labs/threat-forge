...
| 2026-07-21 | `wrangler` lands at 4.113.0, not the planned 4.112.0; exact pin retained | 4.112.0 was already superseded at implementation time. 4.113.0 is at or above `#119`'s target, so supersession still holds. An intermediate revision loosened the pin to `^4.113.0`; preflight review flagged that `wrangler` is one of three deliberately exact-pinned deps (`@tauri-apps/cli`, `fake-indexeddb`) introduced together in `e6a721d` "harden release and dependency controls", and is the tool that deploys to production Cloudflare. The exact pin is restored per `security.instructions.md` ("keep updater, release, and dependency integrity checks fail-closed and pinned"). |
| 2026-07-21 | Step 4 surfaced three Biome 2.5.5 findings, not zero | `assist/source/organizeImports` (export reorder in `src/lib/canvas-utils.ts`) and two `lint/complexity/useOptionalChain` rewrites in `src/components/palette/component-palette.tsx` were applied as the mechanical fixes step 4 authorizes; equivalence of `!canvas \|\| !canvas.contains(target)` → `!canvas?.contains(target)` was checked across all four branches (`canvas` is `Element \| null`; `Node.contains(null)` is `false`). |
| 2026-07-21 | `noImportantStyles` on `src/styles.css` is documented, not suppressed | The rule is **warn** severity and does not fail `biome check`, so a `biome-ignore` would have spent this issue's "no new Biome suppressions" acceptance criterion on non-blocking output. An intermediate revision added one whose stated rationale was **wrong** — it blamed unlayered ReactFlow styles, but xyflow declares no handle `opacity` at all; the real antagonist is this repo's `!opacity-0` in `shared-handles.tsx` emitting `opacity: 0 !important` into `@layer utilities`. Preflight review measured in Chromium that the relocation fix originally proposed in `#134` is broken (unlayered `!important` ranks below layered `!important`). Replaced with a plain CSS comment carrying the verified mechanism; `#134` rewritten with the measurement table and a cascade-correct approach. |
| 2026-07-21 | E2E gate deferred behind `#111` | `#93`'s acceptance criterion "output loads in browser and Tauri webview" is gated on E2E, and E2E is flaky on `main` independently of this change (reproduced on the Vite 6 baseline). This PR rebases onto `#136` and re-runs the E2E gate before merge, so verification is not complete until that run is attached. |
