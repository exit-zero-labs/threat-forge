...
5. **The 300s freshness cap does not hold in the visitor's browser.** Measured: the zone's default
   ...
   **Inferred, not measured — and it should be checked at the same dashboard visit:** whether the
   ...
   citation:** the Cache API reference never mentions `CF-Cache-Status` anywhere, including in the
   Headers section that enumerates what the implementation acts on, and the one Cloudflare
   sentence asserting the header is written about `fetch` read-through. Pushing the other way,
   the round-1 measurement found the `max-age=14400` rewrite on the `HIT` and the Worker's own
   `max-age=300` on a `MISS` — a correlation that would have to be coincidence if the zone's
   cache store were never involved, though `caches.default` alone satisfies "involved". The honest position is
...
5. **Watch one real refusal.** After a day or so of `wrangler tail` or Workers Logs, confirm the
   ...
   one chance to see whether `no-store` survives on a cacheable status, which step 5 and
   `docs/runbooks/deploying-the-website.md` currently infer rather than measure.
...
| 2026-07-27 | Round 3 | `cf-cache-status: HIT` does not evidence a front-of-Worker cache | Reviewer: `cache.match` is documented to add `CF-Cache-Status: HIT` to what it returns, so the live `HIT` and non-zero `age` behind residual risk 5 are fully explained by the Worker's own `caches.default`. The `Cache-Control: max-age=14400` rewrite is unaffected and `#285` stands — but the one measurement that looked like evidence of an independent edge cache is not. Worth having in hand at the dashboard visit. **Withdrawn in round 4 — see the row below. The citation is false and the conclusion drawn from it was too strong in the opposite direction** |
| 2026-07-27 | Round 4 | The `CF-Cache-Status` citation retracted; residual risk 5 taken to genuinely ambiguous | The row above is wrong. `cache.match` is **not** documented to stamp `CF-Cache-Status`: the Cache API reference never mentions the header at all, and the one Cloudflare sentence asserting it is written about `fetch` read-through. The reviewer had sourced it from memory and retracted it unprompted; slop found it independently and added the half neither of us had — the round-1 `MISS`→`max-age=300` / `HIT`→`14400` correlation pushes the *other* way, so retracting the measurement wholesale replaced one overclaim with its mirror image. Residual risk 5 now states both pulls, labels its own reasoning as reasoning rather than citation, and concludes the measurement is ambiguous and only the dashboard resolves it. The runbook bullet that told an operator to read provenance out of a `HIT` is rewritten |
| 2026-07-27 | Round 4 | The signal half of the timeout guard covered by a test rather than argued for | The round-3 commit message and the row above claimed "each half proved load-bearing by deleting it alone". All three lanes measured it: deleting `timeout.aborted` failed nothing, and the uncovered half was the one carrying the point. Closed by substituting an already-aborted signal and naming the rejection something the `name` check cannot match; round 5 then found the first version pinned only the `fetch` catch site, so the test carries both. All four half × site combinations now fail when removed alone |
| 2026-07-27 | Round 4 | The rollback paragraph scoped to what Wrangler actually does | It generalised a `preview_urls`-only mechanism to both subdomain keys. `workers_dev` is recomputed as `config.workers_dev ?? (routes.length === 0)` and sent on every deploy, so reverting the commit does not change it; only `preview_urls` can be omitted, and Wrangler holds two different assumptions about an omitted one in the same file, which is better evidence for "not established" than the single branch first cited |
| 2026-07-27 | Round 4 | Timeout token documented as approximate; cache-key test guarded and de-asserted | The signal is read after the failure rather than at it, so it answers "had the deadline passed" and not "did the deadline cause this" — a request already failing just past the abort lands on the timeout token, and the runbook no longer glosses it as certainty. The bidirectional cache-key case carried no vacuity guard of its own and stated an undocumented consequence (`cache.put` failing off-zone) as fact; it now guards itself, labels its failures with the offending pattern, and says the hazard is a pinned constant ceasing to describe the deployment with the exact consequence unestablished |
