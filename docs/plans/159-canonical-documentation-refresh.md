...
| 2026-07-22 | Tighten false-current claims during self-review | Repository search found no external-file watcher, explicit large-model virtualization, or i18n implementation, while the roadmap assigns external-change handling to Phase 1. Reclassified those product-design statements as planned/target/unscoped, removed unsupported risk precision, and corrected browser-vs-Tauri wording. README edits remain limited to volatile numeric facts and do not perform the #162 migration. |
| 2026-07-22 | Resolve preflight security and domain findings | Qualified browser-versus-desktop key storage and updater-signing status, documented MCP reload and Rust-writer data-loss limits, and corrected lowercase severity values in the canonical `.thf` example. |
| 2026-07-22 | Resolve second-pass security findings | Removed unsupported README audit, updater, sanitization, and path-scoping claims; identified CodeQL as GitHub default setup; and documented native path confinement as an unresolved security review surface rather than a shipped control. |
| 2026-07-22 | Resolve final consistency findings | Corrected README severity casing and unsourced competitor pricing, removed external star-count drift from the glossary, and documented the desktop key-storage threat-model tradeoff. |
