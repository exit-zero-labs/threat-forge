...
|------|--------|---------------------|
| 2026-07-23 (preflight) | Closed teardown resurrection and restore version-parity gaps | Full E2E and independent specialist review found two should-fix gaps. (1) Closing a dirty tab inside the 1 s debounce window tore down the autosave effect and flushed its body after `closeDocumentById` removed the manifest entry; `persistDocument` then unconditionally re-upserted that entry, resurrecting the explicitly closed tab. A completed write now advertises a manifest entry only while the registry still owns the same store bundle. The late flush still retains the latest body as a recoverable #55 orphan, and a deferred-write unit regression deterministically pins the no-resurrection behavior. (2) Restore parsed app-authored IndexedDB bodies with `parseThreatModelYaml` but skipped ADR-009's exact schema-version check, unlike browser file-open, desktop, and AI read paths. The shared exported `validateThreatModelVersion` now rejects an unsupported stored version as corrupt/recovery-needed without applying full reference validation to an in-progress model; restore tests and the architecture handoff record the deliberate lightweight-parser boundary. The slop lane's redundant `hydratedIds` merge parameter was also removed because registry `openDocumentIds` is the hydrated set by invariant. |
| 2026-07-23 (parent final review) | Closed remaining boot and close-pointer races | The final parent pass found that (1) boot restore did nothing when reconciliation left a non-empty manifest with no valid active pointer, (2) a slower boot read could activate the originally persisted tab after the user had already selected another restored tab, and (3) close-neighbour activation relied on a later React effect to persist the settled active pointer. Restore now tries the persisted active id or the first manifest id, hydrates without activation, and claims selection only while no user-selected registry document exists; otherwise it mirrors the user's live choice. Shared close and pre-hydration close paths now persist their settled active pointers synchronously, and asynchronous neighbour loading preserves a later user selection. Discriminating restore/close tests cover missing-pointer fallback, slow-boot-read last-user-choice wins, and the manifest active pointer after neighbour activation. |
| 2026-07-23 (self-review, corrective pass) | Three remaining cross-projection lifecycle defects (same uncommitted change set) | A final anti-slop pass on the still-uncommitted 2026-07-23 work found three concrete, evidence-backed defects the prior self-review missed; each is fixed with the smallest coherent design and a discriminating test, preserving all prior correct behavior. (1) **Outgoing write clobbered the active pointer**: `persistDocument` called `workspace.setActiveDocumentId(id)` unconditionally after its async body write, so on an A→B switch — where B records itself active synchronously in the effect — A's slower flush finishing later overwrote the persisted pointer back to A and reopened the wrong tab on reload. This is a real production race, not a StrictMode dev artifact. The post-write pointer update is now guarded on the registry *still* having that document active (`useDocumentRegistry.getState().activeDocumentId === id`); known-document activation still updates the pointer synchronously in the effect. A deferred-write regression proves A finishing after B cannot steal the pointer, and the `waitForLocalSave` E2E helper comment was rewritten to describe the real durability/settle reason instead of blaming StrictMode. (2) **Reorder used a registry-only index**: keyboard reorder moved a target by `registryIndex + delta` and pointer-drop measured only registry tabs, so with un-hydrated siblings between live tabs (manifest [A(restored),B(restored),C(active)]) moving A one visible position right yielded [B,C,A] instead of [B,A,C]. Both paths now compute the desired order against the merged visible `tabOrder` via the shared `applyVisibleReorder`, enforce the existing pinned/unpinned block with `moveDocumentInOrder`, and project that order coherently into the manifest (whole desired order, so a restored target hydrates at its new persisted slot) and the registry (hydrated-only projection); restored tabs stay non-draggable until hydrated. Discriminating 3-tab keyboard and pointer tests replace the insufficient 2-tab case. (3) **Closing the selected unhydrated boot tab**: before boot hydration resolves the persisted-active tab renders selected but is not in the registry, and `handleClose` treated it like any background restored tab — removing it, focusing New, neither superseding the pending activation nor hydrating a neighbor. It now detects the pending-active boot tab (`registry.activeDocumentId === null && workspace.activeDocumentId === id`), supersedes pending activation, drops its open-manifest entry (retaining the IndexedDB body for `#55`), and hydrates+activates its full-order right/left neighbor from `tabOrder`, focusing it; the removed tab's in-flight boot read still refuses to resurrect it (manifest-entry-gone guard). A deferred-read component test covers close-before-active-hydration, neighbor activation, and late read resolution. `#55` recovery/export and `#63` chat persistence remain untouched; no stored body is ever deleted. |
| 2026-07-23 (self-review) | Cross-projection tab-lifecycle corrections (pre-commit, one uncommitted change set) | A second anti-slop pass on the still-uncommitted 2026-07-23 work found concrete cross-projection lifecycle defects, all fixed here with the smallest coherent design and discriminating tests, preserving prior correct behavior. (1) **First-paint selection/focus fallback**: before the active body finishes async hydration the registry `activeDocumentId` is null, so `DocumentTabStrip` left every manifest tab `aria-selected=false`/`tabIndex=-1`. The strip now falls back to the persisted active pointer for `selectedId`/`effectiveFocusedId` until hydration resolves, then defers to registry state — a delayed-read component test proves exactly one selected and one focusable tab on first paint. (2) **Hydration races**: `handleActivate` fired untracked async reads, so rapid clicks could activate whichever read resolved last; an `activationSeqRef` makes user activation last-request-wins, and `hydrateDocumentById` now captures whether the manifest entry existed at call time and refuses to hydrate a manifest document removed while its body was being read (explicit orphan recovery — no prior entry — still allowed), so closing a restored tab mid-read can no longer resurrect it. Deferred-read tests cover both. (3) **Close semantics at the lifecycle boundary**: hydrated-document manifest removal moved out of `DocumentTabStrip` into the shared `closeDocumentById` (after dirty confirmation succeeds), so File menu, command palette, keyboard/native menu, and every other caller drop the manifest entry too; the strip still removes never-hydrated manifest tabs. The IndexedDB body is still retained for `#55`. `use-file-operations` regression tests added. (4) **Full-order neighbor activation**: after reload the registry may hold only the active document, so closing it saw no registry neighbor; `closeDocumentById` now computes the D1 right/left neighbor from the merged full tab order, hydrates+activates it (no blank fallback), and focuses it — for close buttons and shared close commands alike, with coverage. (5) **Order authority**: `setManifestOrder` reordered the array but left each `WorkspaceManifestEntry.order` stale; a new live document reordered before its first write also appended with the wrong order. `withCoherentOrder` keeps array position and `.order` coherent on every mutation, a new document's initial order derives from current registry order, and upsert sorted-inserts to preserve persisted order — with store/persistence tests. (6) **Pinned/live merge correctness**: `mergeWorkspaceTabs` could thread an un-hydrated manifest tab before a pinned live tab; the pure contract is now a 3-block order (pinned-hydrated lead, manifest-anchored middle, registry-only-new trail) so hydrated pinned tabs stay the leading block, and registry-only new documents follow the persisted manifest when no anchor exists (fixing the mislabelled "after" test). Discriminating pure/component tests added. (7) **Restored-tab controls**: an un-hydrated restored tab is now non-draggable (no silent drag no-op) while keyboard reorder/pin hydrates it first and then performs the action; WAI-ARIA/manual activation and the single shared `DocumentTabView` markup are preserved. (8) **E2E truthfulness**: the post-`page.reload()` spinner-count assertion in `e2e/browser-restore.spec.ts` could not prove a transient blocking spinner never appeared, so it was removed; the deterministic no-blocking-first-paint proof is the delayed-read unit test, and the E2E keeps only stable shell/content assertions. `#55` recovery/export and `#63` chat persistence remain untouched. **This entry supersedes the 2026-07-22 row item (7): a hydrated document's open-manifest entry is now removed on close (only its IndexedDB body is retained for `#55`), rather than kept, so it can neither render as a restored tab nor resurrect on reload.** |
| 2026-07-23 | Closed the #54/#56 manifest→registry tab-rendering seam | The strengthened `e2e/browser-restore.spec.ts` exposed a must-fix integration defect from the prior handoff: `useWorkspaceRestore` correctly hydrates only the active document and leaves siblings as manifest descriptors (D4), but `DocumentTabStrip` rendered only `useDocumentRegistry.openDocumentIds`, so after a reload every inactive persisted document was invisible even though its body remained in IndexedDB — defeating #56's own objective ("a browser restart restores the previous workspace") and #48's exit gate. Because #54 is closed, that seam it was expected to reconcile had no owner; #56 must close it. Minimal integration, preserving lazy hydration: (a) new pure `src/lib/workspace-tabs.ts#mergeWorkspaceTabs` unions the hydrated registry sessions with the persisted-but-un-hydrated manifest documents into one persisted order; (b) `document-tab.tsx` was refactored to share one presentational `DocumentTabView` between the live `DocumentTab` and a new `RestoredDocumentTab` (no duplicated tab markup, same WAI-ARIA/drag contract); (c) `DocumentTabStrip` renders the merged list, activates an un-hydrated tab through the existing `hydrateDocumentById` (selecting it only on success — a failed read stays fail-visible and opens no blank document), drops a closed tab's manifest entry so it never resurrects while its IndexedDB body is retained for `#55`, and mirrors live reorder/pin onto the manifest order; (d) `hydrateDocument` gained an optional `insertIndex`, computed from the manifest in `hydrateDocumentById`, so activating a restored tab lands the document at its persisted slot instead of jumping it to the end of the registry (the exact order regression the E2E caught). No stored body is ever deleted; #55 still owns recovery/export; #63 chat persistence untouched. The step-8 E2E, previously deferred (see the 2026-07-22 row, item 8), is now written and proven to fail on the pre-fix strip (only one tab renders after reload). |
| 2026-07-22 | Steps 6–10 implementation deviations | (1) **`captureCanvasIntoModel` takes its canvas state as a parameter** instead of moving verbatim. Reading `useCanvasStore.getState()` inside it reads the *active* facade, so a debounced write that outlives a document switch would fold the wrong document's geometry into the model. `use-file-operations.ts` passes the active canvas at the same call sites, so file-save behavior is unchanged. (2) **`hydrateDocument` honors `activate` for an already-open document** rather than being a total no-op: the bundle is still never rebuilt (stored text never overwrites live work), but "make this restored document current" is what the caller asked for. (3) **`WorkspaceManifestEntry` gains a required `createdAt`**, without which D4's "rebuild under the persisted `createdAt`" cannot hold — the manifest previously stored only `updatedAt`, which is a modification time. (4) **`DocumentPersistenceState` gains an optional `errorKind`** so the indicator can distinguish a full quota from a generic failure, as D5 requires. (5) **`useWorkspaceRestore` branches on `isTauri()`** in addition to the storage factory: `isAvailable()` alone cannot tell "desktop, not applicable" from "browser, storage blocked", and only the second may show the ephemeral notice. (6) **Autosave seeds a document that has no manifest entry** on bind, so a New/Open/Import/template document survives a reload without an edit; a document already in the manifest is not rewritten at boot, so restoring costs no revision. (7) **The persisted active pointer is cleared when no document is open**, so closing a document does not resurrect it on reload; its manifest entry and stored body are kept for `#55`. [Superseded by 2026-07-23 (self-review): a hydrated document's open-manifest entry is now **removed** on close so it cannot render as a restored tab or resurrect; only the IndexedDB body is retained for `#55`.] (8) **`e2e/browser-restore.spec.ts` was not written** — the Playwright browser lane was outside this run's verification scope; the restore, non-blocking-paint, and failure paths are covered by `src/hooks/use-workspace-restore.test.ts`, and the E2E spec remains required before handoff. |
