...
| 2026-07-21 | A **second** readiness race was found during implementation and is fixed in `#136` | The plan diagnosed only the canvas mount race. The observed blocker is `dfd-basics`, which auto-starts 800ms after first model creation and renders a full-viewport `guide-overlay` intercepting pointer events; probed against the pre-fix seeding, `palette-item-generic.dblclick()` times out. The suite was passing only by out-racing that timer, which a saturated runner defeats — so this is plausibly a real contributor to the CI flake. Closing the canvas race (which legitimately spends ~300ms) exposed it. Correction from preflight review: an earlier revision of this row also blamed the `welcome` guide. `welcome` is in fact inert in the E2E environment — `main.tsx` enables StrictMode, whose double-invoked effect clears the 500ms timer while the `firstLaunchChecked` ref prevents rescheduling (probed 3/3: no tooltip or overlay after 1500ms idle). It is still seeded, because that is a dev-only accident rather than a guarantee, and seeding the What's New key does make `isWhatsNewVisible()` return false and thereby enable it in a production-like build. The dev/prod divergence is filed as `#142`. |
| 2026-07-21 | Auto-start suppression works at the trigger hook, not through `showOnce` | Second correction from preflight review. `use-onboarding-triggers.ts:42,63` computes `alreadySeen` from `completedGuideIds`/`dismissedGuideIds` and skips scheduling the timer entirely; `startGuide`'s `showOnce` guard governs only manual starts from the guide picker. An intermediate revision of `e2e/fixtures.ts` told maintainers to "confirm the guide is `showOnce: true`", which was both wrong and hazardous — it could push someone to change a product flag to make E2E pass. |
...
| 2026-07-21 | Auto-start suppression is a real coverage gap, not covered by unit tests | Preflight review corrected an earlier claim that `guide-overlay.test.tsx` retains the coverage. It tests only the presentational component, and `use-onboarding-triggers.test.ts` calls `startGuide` directly rather than rendering the hook, so the 500ms/800ms timers and `isWhatsNewVisible()` are untested in both lanes. Filed as `#141` — a plan row alone would be the Markdown-only backlog `AGENTS.md` forbids. |
