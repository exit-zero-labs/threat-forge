...
Need to develop a comprehensive icon library for components on the canvas. Icons for all offerings of major services, technologies, platforms, and tools should be included. They should be very easy to select for a component on the canvas (and connectors for technologies that integrate with other technologies) and should be visually consistent and recognizable. Custom icons should be supported but only by contributing to the open source repo itself (no user uploads) to ensure quality and consistency. The icon system should be designed for extensibility to allow adding new icons over time as needed.
...
## 2026-03-01 - Native Titile Bar Controls

Add native controls to title bar to enable actions. These should also be secondary. Every control should be always be designed primarily via surface access first but also included in the title bar controls for windows/mac/linux etc.

## 2026-03-01 - Cmd+K Command Bar

A super fast way to add components and do actions. Can type things like library component names to add to canvas, actions to do like open panes/navigate/save etc etc (must match pre-defined operations in command palette). UI would be a MacOs Spotlight style search bar appearing as overlay. Also an update to the component library in the left. A double click on any component in the library would add it to the canvas. This is a much more intuitive way to add components to the canvas and discover the available components.

## 2026-03-01 - Move position and almost everything else to yaml

Lets rethink what goes into yaml and what doesnt. Lets move everything into the yaml file. Including positions. We can just try to mold the format such that it still remains easy to ready. The threat model file needs to be extremely portable between users whether web or desktop. It should server as the complete exact copy on another user’s machine as it does on one user’s machine.

## 2026-03-01 - Change in component library selection presentation

Instead of the current ‘process’, ‘data store’, and ‘entity entity’, ‘trust boundry’ lets just have two: “Entity” and “Boundry”. Properties panel will let user change the entity between things like database, web service, api, etc etc (research all the different extensive options that should be here) - also changing its shape. Users can also change its fill and store (with opacity) similar to threat boundry. Keep the left pane divided into two. Top part will have the entity and boundry items. Bottom pane will be library. The library is a searchable list of pre-crafted items of various technologies, platforms with icons etc etc. At the top there should be a small search bar to quickly and fast search an item to add like Azure Key Vault or React App etc. Need to create this extensive library. For pre-fabricated items, they cannot change things like color (fill, stroke), entity type but can change the label. They can also do a “swap” to another entity seamlessesly without losing position and connections.
The 3P services/component library should be designed as horizontally scrollable tabs in the top bar of the section divided by things like Azure, AWS, Languages, Dev Tools, etc etc. Clicking on a tab will show the items for that category in the library section below. This will allow us to have a very extensive library without overwhelming users. There should also be an 'All' tab to show everything with sections divided by subtle headers of horizontal sections.

## 2026-03-01 - More keyboard shortcuts

More shortcuts to add. Mac: Cmd+B for left panel. Cmd+I for right panel. Cmd+Shift+I for AI tab. Cmd+0 for fit to view. “plus”/”minus” buttons to zoom in and out of graph. If something is selected, then arrow keys to move it on the graph. If nothing is selected, the pan the graph. Cmd+L to lock the graph. Also, Option+Click and drag to duplicate. Cmd+C/X/V functionality for nodes, edges and connector (for connectors, properties only not the positioning). Cmd+Z/Y for history. Need to keep the last 20 actions in local cache.

## 2026-03-01 - MCP Integration

Add support for MCP driven commands from tools like Claude or Githb Copilot. Must be local MCP.

## 2026-03-01 - SEO optimization

Optimize website and documentation for search engines. Add proper meta tags, structured data, and ensure fast load times.
