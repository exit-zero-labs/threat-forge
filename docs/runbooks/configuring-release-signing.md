...
| Windows | `src-tauri/tauri.signing.conf.json`, `scripts/sign-windows.ps1`, and `src-tauri/trusted-signing-metadata.json` define a custom SignTool path for account `threatforge-signing`, profile `threatforge-public`, and East US. Azure credential names exist as repository secrets. The workflow downloads the legacy Trusted Signing client directly from NuGet and verifies its pinned package hash before loading the signing DLL. **The signing path is disabled**: both the client download and the `--config` flag are gated on the `WINDOWS_SIGNING` repository variable, which is unset. | Enabling it during the v0.3.0 release failed the Windows build outright, and the diagnosis on [#50](https://github.com/exit-zero-labs/threat-forge/issues/50) records three defects in the sign command and script. Those, plus Azure resource and publisher-identity verification, the migration to the current Artifact Signing client, and the move off a client secret to OIDC, all remain. |
