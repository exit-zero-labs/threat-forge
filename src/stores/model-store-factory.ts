...
export interface ModelState {
	...
	/** Reset the history-capture debounce so the next edit starts a new undo step. */
	resetCaptureDebounce: () => void;
...
/** Create a threat-model store bound to one document's history store. */
export function createModelStore(deps: ModelStoreDeps): StoreApi<ModelState> {
	// Debounce state is per document: grouping rapid edits into one undo step must not be
	// influenced by edits made in another document.
	...
	return createStore<ModelState>((set, get) => ({
		model: null,
		filePath: null,
		isDirty: false,
		...
		resetCaptureDebounce: resetDebounce,
